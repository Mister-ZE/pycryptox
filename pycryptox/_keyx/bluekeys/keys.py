"""
Keyx BLUE keys:
Encrypted store for BLUE channel records, sealed in a `.purple`
file with the PURPLE protocol. Each entry is a (name, channel)
pair where channel is `{mygpubkey, hgpubkey, privkey}`. The store
is opened as a session via `open()` and operations stay in memory
until session close.
"""
...
# Functions:
_MAGIC = b"CRXKX_BLUEKEYS_1"
_REQUIRED_EXTENSION = ".keys"
_FORMAT_TAG = "cryptox-keyx-bluekeys"
_FORMAT_VERSION = 1
_REQUIRED_FIELDS = frozenset({"mygpubkey", "hgpubkey", "privkey"})


def _validate_entry(entry: dict[str, str]) -> None:
    if set(entry.keys()) != _REQUIRED_FIELDS:
        raise KeystoreError("entry must have fields: mygpubkey, hgpubkey, privkey")
...
class _KeysSession:
    ...
    def __enter__(self) -> "_KeysSession":
        self._check_open()
        return self
    ...
    def get(self, name: str) -> dict[str, str]:
        """Return the entry for `name` as a dict with `mygpubkey`, `hgpubkey`, `privkey`."""
        self._check_open()
    ...
    def add(self, name: str, mygpubkey: str, hgpubkey: str, privkey: str) -> None:
        """Add a new entry."""
        self._check_open()
        ...
        entry = {"mygpubkey": mygpubkey, "hgpubkey": hgpubkey, "privkey": privkey}
    ...
    def update(
        ...
        if name not in self._entries:
            raise KeyNameError(f"no entry named '{name}'")
        if mygpubkey is None and hgpubkey is None and privkey is None:
            return
...
def open(password: str, dbpath: str | os.PathLike) -> _KeysSession:
